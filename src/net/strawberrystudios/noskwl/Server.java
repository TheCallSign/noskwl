...
//    private static final List<ClientWorker> clientList = new ArrayList();
    ...
    public synchronized void setClientUsername(ClientWorker cw, String userID) {
    ...
    /**
     * DO NOT RUN THIS METHOD DIRECTLY TO START THE SERVER
     * Use listen(int port) instead. Running this will cause an exception.
     */@Override
     public void run() {
         
         if(this.port == 0){
             try {
                 throw new IllegalPortException("Port has not been set, please use server.listen(int) to start the server");
             } catch (IllegalPortException ex) {
                 Logger.getLogger(Server.class.getName()).log(Level.SEVERE, null, ex);
             }
         }
         
         stdout("NoSkwl Server version " + Server.VERSION + "."
     ...
     private void serverLoop() {
         while (true) {
             ...
             try {
                 ...
                 stdout("Client connected with UID: " + UID);
